...
class Gender(models.TextChoices):
    MALE = "Male", _("Male")
    FEMALE = "Feale", _("Feale")


class Profile(TimeStampedUUIDModel):
    user = models.OneToOneField(User, related_name="profile", on_delete=models.CASCADE)
    phone_number = PhoneNumberField(
        verbose_name=_("Phone Number"), max_length=30, default="+48123123123"
    )
    ...
    is_agent = models.BooleanField(
        verbose_name=_("Agent"), default=False, help_text=_("Are an agent?")
    )
    top_agent = models.BooleanField(verbose_name=_("Top Agent"), default=False)
    ratinig = models.DecimalField(max_digits=4, decimal_places=2, null=True, blank=True)
